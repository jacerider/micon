  ...
  Plugin.prototype = {
    ...
    init: function () {
      ...
      this.iconContainer.on('click', '.fip-box', $.proxy(function (e) {
        this.setSelectedIcon($(e.currentTarget).find('.micon').attr('data-fip-value'));
        this.toggleIconSelector();
      }, this));
    ...
    renderIconContainer: function () {
      ...
      for (var i = 0, item; item = iconsPaged[i++];) {
        ...
        $('<span/>', {
          ...
          'class': 'fip-box',
          title: item
        }).appendTo(this.iconContainer).children(":first").attr('data-fip-value', item);
      }
    ...
    getIconTitle: function (theIcon) {
      ...
      return $('<div/>').html(JSON.parse(iconTitle)).text();
    },
    ...
    setSelectedIcon: function (theIcon) {
      ...
      if (theIcon) {
        this.iconPicker.find('.selected-icon').html(this.getIconTitle(theIcon));
      } else {
        this.iconPicker.find('.selected-icon').html('<i class="fip-icon-block micon"></i>');
      }
